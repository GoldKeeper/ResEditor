...
{
	saveAction = ui.mainToolBar->addAction(QPixmap(":/ResEditor/Resources/imgcuter/disk.png") ,"Сохранить");
connect(saveAction,SIGNAL(triggered()), this, SLOT(saveData()));
	textureUpdate = ui.mainToolBar->addAction(QPixmap(":/ResEditor/Resources/imgcuter/arrow-circle-double.png"),"обновить текстуру");
connect(textureUpdate,SIGNAL(triggered()),this,SLOT(slotTextureUpdate()));
	layerChes = ui.mainToolBar->addAction(QPixmap(":/ResEditor/Resources/imgcuter/layer-transparent.png"),"Шахматный фон");
connect(layerChes,SIGNAL(triggered()),this,SLOT(slotLayerChes()));
	layerChes->setCheckable(true); 
	layerChes->setChecked(true);
	zoomIn = ui.mainToolBar->addAction(QPixmap(":/ResEditor/Resources/imgcuter/magnifier-zoom-in.png"),"Увеличить");
connect(zoomIn,SIGNAL(triggered()),this,SLOT(slotZoomIn()));
	zoomOne = ui.mainToolBar->addAction(QPixmap(":/ResEditor/Resources/imgcuter/magnifier-zoom-actual.png"),"Масштаб 1:1");
connect(zoomOne,SIGNAL(triggered()),this,SLOT(slotZoomOne()));
	zoomOut = ui.mainToolBar->addAction(QPixmap(":/ResEditor/Resources/imgcuter/magnifier-zoom-out.png"),"Уменьшить");
